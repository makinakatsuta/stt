...
以下は今回の実装状況です。過去の項目は当時の記録として保持します。

- [x] 68.1 **簡単**：サーブを除く双方の返球を合計4回までに制限し、CPUが取り逃す形で終了。CPUの再抽選を1球1回に制限。
- [x] 68.2 **ハード**：CPUだけが速球・遅球を選択。プレイヤーの移動・返球は手動のまま維持。
- [x] 68.3 **ノーマル**：中央・左右・空いた側へのコース選択、通常球と遅球、CPUの必要移動量に応じた返球率を実装。回数による強制終了は設けない。
- [x] 68.4 **物理処理**：ノーマルの予測移動・返球計画・1球1回の判定をJS/WASMで統一。CPUがP1側の場合の追従も修正。ノーマルのWASMだけにあったエンドラインのランダムなアウト判定を除去。
- [x] 68.5 **案内**：ノーマルの短い振り返り、オン・オフと保存、読み上げ用の待ち時間を追加。トップ画面と難易度選択の説明を更新。
...
- [ ] 68.8 **実プレイでの難易度調整**：初心者・経験者それぞれで勝率、返球成功率、ラリー回数を測定する。ノーマルの双方合計6〜10回は目標に留まり、現時点では達成確認なし。自動で上限を設けずに調整する。
- [ ] 68.9 **操作・音響の実機確認**：キーボード、スマートフォンの加速度操作、ヘッドホンで3難易度を通して遊ぶ。ハードの緩急が聞き分けられるか、ノーマルの左右への移動が無理なくできるかを確認する。
- [ ] 68.10 **読み上げと表示**：VoiceOver・TalkBack・NVDA等でトップの説明、振り返りのオン・オフ、得点コールとの重複、スキップ・中断時の読み上げ、拡大表示を確認する。66.5の表示検証も継続。
- [x] 68.11 **設定保存の例外処理**：共通の保存処理を追加。振り返り・難易度・音声速度・センサー等の設定は、保存拒否時もセッション内で利用可能。読み取り拒否と書き込み拒否をテスト済み。
...
2026-09-23追加確認：ノーマルを仮想プレイヤーで各200ラリー計測し、平均返球回数はJS 7.03、WASM 6.81（中央値は両方7）。実プレイの代用ではないため68.8は未完了。操作可能なブラウザがこの実行環境に存在せず、66.5・68.9・68.10も未完了のままとする。
