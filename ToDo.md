...
2026-09-24の再チェック：全盲向けを評価対象とする。Windows Firefox・Chrome・Edge、Android Chromeはユーザーによるプレイ検証済み。ロービジョン向け検証（66.5や拡大表示）は今回の評価対象外として保留する。以下の過去の未確認記述より、この検証状況を優先する。詳細と改善事項は [accessibility-review.md](accessibility-review.md) を参照。

サウンドテーブルテニス（STT）の公式ルールに準拠し、Go（WebSocket）バックエンドとWeb Audio API（フロントエンド）を組み合わせたブラウザゲームの開発ToDoリストです。オンライン対戦は現在一時停止中です。

## 68. 難易度の役割整理と試合練習の改善（v3.31.22／2026-09-23）

2026-09-24の追加調整は確定版v3.31.25に収録。

以下は2026-09-24の調整・ユーザーによるブラウザ検証まで反映した実装状況です。過去の項目は当時の記録として保持します。

- [x] 68.1 **簡単**：低速・広い返球判定を維持し、サーブを除く双方の返球上限を合計4回から8回へ延長。CPUの最初の返球ではランダムなミスをなくし、以降の成功率を90％に変更（ラケットが届いた場合）。CPUの判定は1球1回。
- [x] 68.2 **ハード**：CPUだけが速球・遅球を選択。プレイヤーの移動・返球は手動のまま維持。
- [x] 68.3 **ノーマル**：移動距離と到達までの時間に応じて返球率・球速・コースを決定。余裕があれば空いた側を狙い、追い込まれると中央寄りへ弱めに返す。急な球速変化・極端なCPUサーブ・CPUの細かな左右の揺れを抑制。ラリー回数によるミス増加と強制終了は設けない。
- [x] 68.4 **物理処理**：ノーマルの予測移動・返球計画・1球1回の判定をJS/WASMで統一。CPUがP1側の場合の追従も修正。ノーマルのWASMだけにあったエンドラインのランダムなアウト判定を除去。
- [x] 68.5 **案内**：ノーマルの得点後の振り返りとオン・オフ設定を廃止。全難易度で判定・得点者・スコアの審判コールと得点後の待ち時間を統一。トップ画面と難易度選択の説明を更新。
- [x] 68.6 **検証**：簡単・ハードのロジックテスト、ノーマルのJS/WASM統合テストを追加。ビルド・バージョン確認の結果はリリースノートに記録。
- [x] 68.7 **資料**：READMEを現行仕様で再構成。v3.31.22の履歴を追加し、ソース・画面・読み込みURLの番号を統一。

### 検証状況・次に確認すること

- [ ] 68.8 **実プレイでの難易度調整・測定**
  - [x] 簡単をラリー重視、ノーマルを自然な試合重視に調整。2026-09-24、ユーザーが今回の変更をブラウザで検証し、遊べることを確認済み。
  - [ ] 初心者・経験者それぞれで勝率、返球成功率、ラリー回数を測定する。ノーマルは固定回数を目標に終了させず、コースとタイミングに応じてラリーが続く現行方針で評価する。
- [ ] 68.9 **操作・音響の実機確認**
  - [x] iPhone・Android・Windowsのブラウザで、スクリーンリーダーとヘッドホンを使用してプレイできることをユーザーが確認済み（2026-09-24）。
  - [ ] 入力方式別（キーボード・スマートフォンの加速度操作）の3難易度の詳細確認。ハードの緩急の聞き分け、ノーマルの左右移動の操作感を記録する。
- [ ] 68.10 **読み上げと表示**
  - [x] 振り返りコメントと設定項目を廃止し、審判コールを統一。旧設定がオン・オフのどちらでも全難易度のコールが一致することを自動テストで確認。ユーザーによる今回の変更のブラウザ検証も完了（2026-09-24）。
  - [x] iPhone・Android・Windowsそれぞれで、スクリーンリーダーを使ったブラウザでのプレイをユーザーが確認済み（2026-09-24）。製品名・バージョンは指定なし。
  - [ ] トップの説明、得点コールの重複、スキップ・中断時の読み上げの個別確認と拡大表示を確認する。66.5の表示検証も継続。廃止した振り返りのオン・オフ確認は対象外。
- [x] 68.11 **設定保存の例外処理**：共通の保存処理を追加。難易度・音声速度・センサー等の設定は、保存拒否時もセッション内で利用可能。読み取り拒否と書き込み拒否をテスト済み。振り返り設定は2026-09-24に廃止。
- [x] 68.12 **物理処理の統一**：全難易度でCPU予測移動を使用。簡単の手動移動は8.5に統一し、WASMにもセンサーの移動割合を適用。WASMだけのランダムなエンドライン判定を除去。ハードも1球1回の返球判定（成功率88％）にし、プレイヤー返球時にリセット。先行入力の処理順も統一。
- [x] 68.13 **回帰テストの拡充**：全難易度・両側のJS/WASM移動、返球、失点、サーブ時の初期化を検証。設定保存復元・保存拒否・起動・難易度選択・中断・ヘルプ・再開・ゲーム交代のテストを追加。
- [x] 68.14 **配布・公開**：v3.31.22のexeとdocsを同梱したZIPを作成。展開先のexeを別の作業ディレクトリから起動し、HTML・JS・WASM・音源のHTTP 200応答とバージョンを確認。コミット・タグとGitHub Releaseで配布（https://github.com/makinakatsuta/stt/releases/tag/v3.31.22）。

2026-09-23時点の記録：ノーマルを仮想プレイヤーで各200ラリー計測し、平均返球回数はJS 7.03、WASM 6.81（中央値は両方7）。当時は実ブラウザ確認が未完了だった。

2026-09-24更新：ユーザーから、今回の簡単・ノーマル調整と得点後コメント廃止をブラウザで検証し、遊べることを確認済みとの報告を受領。さらにiPhone・Android・Windowsの各ブラウザで、スクリーンリーダーとヘッドホンを使用してプレイできることを確認済みとの追加報告を受領し、68.9・68.10の該当部分をチェック済みにした。全難易度のロジック・JS/WASM統合・設定テストも成功。調整後の仮想プレイヤー各200ラリーの平均返球回数はJS 17.91、WASM 18.13。これは実プレイヤーの測定値ではない。68.8の属性別測定、68.9の入力方式・難易度別の詳細確認、68.10の場面別読み上げ・拡大表示、66.5の表示・当事者確認は残作業として保持する。

## [x] 67. 実行ファイルの配布とバージョン管理の整理 (2026-09-23)

- [x] Gitに残っていたstt.exeの追跡を解除し、実行ファイルはGitHub Releasesで配布する構成にした。
- [x] 公開済みタグを変更せず、配布整理版v3.31.21としてソース・画面・exeのバージョンを統一した。

## 66. ロービジョン向け表示設定と起動画面の整理 (2026-09-23)

- [x] 66.1 **[画面] 狭い画面での情報表示** (`docs/styles.css`)
  - 得点・審判コール・操作説明の非表示を解除し、得点欄の幅と折り返しを調整した。
- [x] 66.2 **[設定] 配色と文字サイズの選択** (`docs/index.html`, `docs/styles.css`, `docs/js/display-settings.js`)
  - 起動画面にコンボボックスを追加した。配色は標準・黒背景白文字・白背景黒文字・黒背景黄色文字、文字サイズは100%・125%・150%・200%から選べる。
  - 選択を即時反映し、保存・次回復元に対応した。保存不可の場合も設定変更は利用できる。
  - 文字サイズはメニュー・得点・案内に適用し、コート内の文字サイズや当たり判定は変更しない。
  - 標準以外の配色では装飾の発光を抑え、コート表示を白黒でコントラスト調整する。
- [x] 66.3 **[配置] 音声の速さ設定を開始ボタンの直前へ移動** (`docs/index.html`, `docs/styles.css`)
  - 配色・文字サイズ、音声の速さ、開始ボタンの順に配置した。
- [x] 66.4 **[検証] 構文・設定処理・差分チェック**
  - JavaScript構文、全選択肢の反映・保存・復元、不正な保存値と保存不可の場合、`git diff --check`を確認した。
- [ ] 66.5 **[検証] 実ブラウザと当事者による確認**
  - 各配色・文字サイズ、狭い画面、ブラウザ拡大時の表示とキーボード操作を確認する。
  - ロービジョンの当事者による読みやすさとプレイの確認を行う。
- [x] 66.6 **[配布準備] v3.31.20への統一とWindows x64版ビルド**
  - ソース・画面・READMEのバージョンと更新ファイルのキャッシュ識別子を統一した。
  - `go test ./...`（テストファイルなし）、`go build -trimpath -o stt.exe main_server.go`が成功し、`stt.exe --version`でv3.31.20を確認した。

## [x] 65. ラケット打球音の追加音量調整 (2026-09-15)

転がり音に負けないよう、ラケット打球音だけを基準値から合計25%引き上げた。プレイヤーとCPUの音量係数を個別に調整し、他の音源・処理は変更していない。
- [x] 65.1 **[音響] ラケット打球音の音量調整** (`docs/js/sound-system.js`)
  - プレイヤーを3.85、CPUを4.081に設定した。
- [x] 65.2 **[配布] 実行ファイルの再ビルド** (`stt.exe`)
  - 最新ソースからWindows版実行ファイルを再ビルドした。
- [x] 65.3 **[検証] 構文・テスト・差分チェック**
  - `node --check docs/js/sound-system.js`、`go test ./...`、`go build -o stt.exe main_server.go`、`git diff --check`を実施した。

## [x] 64. ゲーム中断時の音声停止と再開 (2026-09-13)

「ゲームを中断して戻る」ボタンまたはEscapeキーで確認画面を開いた際に、ゲーム中の連続音を完全停止する処理を追加した。ラリー中にプレーを再開した場合は、玉の音とラリー音を再開する。
- [x] 64.1 **[音響] ゲーム中断時の連続音停止** (`docs/js/sound-system.js`, `docs/js/game-engine.js`)
  - 玉の転がり音、ラリー音、サーブ時の転がり音、ラケット移動音、体育館の環境音を即時停止する共通処理を追加した。
- [x] 64.2 **[確認] 中断後の音声再開** (`docs/js/game-engine.js`)
  - ラリー中にキャンセルしてプレーを再開した場合のみ、玉の音とラリー音を再開する。
- [x] 64.3 **[検証] 構文・差分チェック**
  - `node --check docs/js/game-engine.js`、`node --check docs/js/sound-system.js`、`git diff --check`を実行した。

## [x] 63. 難易度別ラリー時間と促進ルールの調整 (2026-09-12)

Easyの初心者向けコンセプトを維持し、Normalは平均的な選手、Hardは強い選手を想定したラリー時間に調整した。あわせて、公式ルールに基づくゲーム単位の促進ルールを実装した。
- [x] 63.1 **[ゲームバランス] 難易度別ラリー速度の再調整** (`docs/js/game-engine.js`, `main_wasm.go`, `docs/main.wasm`)
  - Easyは既存のラリー速度を維持した。
  - Normalは標準係数1.0とし、平均的な選手を想定したラリー時間にした。
  - HardはNormalより速い係数1.1とし、強い選手を想定した早い試合展開にした。
  - JavaScriptとWebAssemblyのCPU返球処理で速度設定を同期した。
- [x] 63.2 **[公式ルール] 促進ルールの適用** (`docs/js/game-engine.js`)
  - 1ゲーム開始から10分経過後、公式ルールの例外スコアでなければ促進ルールを適用する。
  - 9対9、または10対8以降の終盤スコアでは促進ルールを適用しない。
  - 促進ルール適用中は、レシーバーの7回返球成功でラリーを終了する。
  - 一度適用した促進ルールは、同じマッチの残りゲームにも引き継ぐ。
- [x] 63.3 **[確認]**
  - `node --check docs/js/game-engine.js`、`go test ./...`、WASMビルド、`git diff --check`を実施した。
  - ゲームコンセプトとサウンド処理を変更していないことを確認した。
...
- [x] 61.1 **[ゲームバランス] Normal・Hardのサーブと返球速度** (`docs/js/game-engine.js`, `main_wasm.go`)
  - Normalのサーブ速度を約10%引き上げ、高速サーブ率を15%に設定した。
  - Hardの高速サーブ率を35%に設定し、返球後の縦速度をNormalより速くした。
- [x] 61.2 **[ゲームバランス] Easyの初心者向け速度調整** (`docs/js/game-engine.js`, `main_wasm.go`)
  - Easyのサーブ速度を3.2〜4.5、ラリー速度をNormalの85%に調整した。
  - Easyのラケット移動速度を8.5px/フレーム、CPU返球率を60%に調整した。
- [x] 61.3 **[物理] 返球後の直進と失速防止** (`docs/js/constants.js`, `docs/js/game-engine.js`, `main_wasm.go`, `docs/main.wasm`)
  - 全難易度で返球後の横速度を0にし、相手側エンドフレームへ直進させるようにした。
  - ラリー中の摩擦減衰を停止し、フレーム到達前の失速を抑えた。
- [x] 61.4 **[確認]**
  - JavaScript構文チェック、`go test ./...`、WASM再ビルド、`git diff --check`を実施した。

## [x] 59. 音源差し替え・審判コール表記の調整 (2026-09-05)

差し替えた移動音源を反映し、ストップボールの審判コール表記を整理した。
- [x] 59.1 **[音響] RacketLeftRight.m4a の差し替え** (`docs/sounds/RacketLeftRight.m4a`, `docs/js/sound-system.js`)
  - 新しい音源ファイルに差し替え、ブラウザキャッシュ対策のバージョンを更新した。
- [x] 59.2 **[審判コール] コートストップ表記の変更** (`docs/js/game-engine.js`)
  - 「前コートストップ」を「コートストップ」に変更した。
- [x] 59.3 **確認**
  - `node --check` と `git diff --check` を実施した。

## [x] 60. 試合中断時のステージ選択と再開導線の整理 (2026-09-08)

試合中断時に戻り先の難易度を直接選択できるようにし、キャンセルで現在の試合を再開できるようにした。
- [x] 60.1 **[画面] 中断確認画面のステージ選択** (`docs/index.html`, `docs/styles.css`)
  - 簡単（Easy）・普通（Normal）・難しい（Hard）の3つの選択肢を表示する。
  - 一番下に「キャンセル」を配置する。
- [x] 60.2 **[ゲーム制御] 選択ステージへの復帰と試合再開** (`docs/js/game-engine.js`)
  - 選択した難易度で新しいCPU戦を開始する。
  - キャンセル時は停止時点の状態からゲームを再開する。
- [x] 60.3 **[確認]**
  - `node --check` と `git diff --check` を実施した。
...
- [x] 起動画面・中断メニューに読み上げ方式（内蔵音声／スクリーンリーダー）を追加。保存拒否時もセッション内で利用可能。
- [x] 審判コールの通知先を一本化。保留中の古い通知は、新しい案内・停止・方式変更で取り消す。
- [x] スクリーンリーダーのクリック操作に対応した「サーブ・返球」ボタンを追加。タップ経路との二重入力を防止。
- [x] ボタン上のSpace操作を優先し、中断メニューのフォーカス移動に新しい選択欄を含める。
- [x] アクセシビリティの自動テストを追加し、通知先・保存・取り消し・入力重複・キーボード操作・中断画面のフォーカス循環を確認。
- 確認済み範囲の補足：変更前の版は、全盲の利用者によるWindows Firefox・Google Chrome、Android Google Chrome、iPhone Safariでの動作確認済み。ブラウザとスクリーンリーダーのバージョンは未記録。
- [x] 今回の変更後、Windows版で遊べることをユーザーが確認。今回使用したブラウザ・スクリーンリーダーの種類とバージョン、個別機能の確認範囲は未指定。
- [ ] Android・iPhoneで今回の追加機能を実機確認する。Windowsを含め、準備・返事・サーブ・返球・得点・中断・再開の場面別に通知の重複と操作の成否を記録する。自動テストは実際の読み上げやブラウザのイベント生成を保証しない。

## 全盲向け再チェックの修正（2026-09-24、v3.31.27）

- [x] 得点後・CPU打球時の直接ライブ通知を除去し、審判コールと打球音に統一。
- [x] 中断時にゲーム用の遅延処理を保留し、再開時に残り時間から継続。終了・新試合・スキップ時に取消。
- [x] 内蔵音声の同期・非同期エラーで代替通知。停止・方式変更後の古い通知を破棄。
- [x] 回帰テストを追加し、アクセシビリティ・設定・全難易度の5テストが成功（JS/WASM統合を含む）。

詳細は [accessibility-review.md](accessibility-review.md)。今回の修正後の実機読み上げは未実施。ロービジョン向けは今回の対象外。

## 単体実行ファイル配布（v3.31.27）

- [x] 画面・JS・WASM・音源・ライセンスをstt.exeへ埋め込み、外部docsフォルダーを不要化。
- [x] 埋め込み資産のHTTP応答、WASM MIME型、開発ファイル非配信をGoテストで確認。
- [x] 旧配布作業ツリー、未使用音源、作業用ルールブックを整理。
