...
2026-09-24の再チェック：全盲向けを評価対象とする。Windows Firefox・Chrome・Edge、Android Chromeはユーザーによるプレイ検証済み。ロービジョン向け検証（66.5や拡大表示）は今回の評価対象外として保留する。以下の過去の未確認記述より、この検証状況を優先する。
...
今回の修正後の実機読み上げは未実施。ロービジョン向けは今回の対象外。
